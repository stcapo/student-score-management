  ...
  const fetchNotification = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // 格式化日期时间
        let expireAt = notification.expireAt ? new Date(notification.expireAt) : null;
        if (expireAt) {
          expireAt = expireAt.toISOString().substring(0, 16); // 格式化为 YYYY-MM-DDTHH:MM
        }
        ...
        setError(data.message || '获取通知数据失败');
      }
    } catch (error) {
